...
const DepthText = ({
  ...
  autoOrbit = true,
  orbitSpeed = 0.25,
  fontSize = 'clamp(5.5rem, 24vw, 16rem)',
  fontWeight = 700,
  ...
  const safeTilt = clamp(Number(tilt) || 0, 0, 15);
  const safeSmoothing = clamp(Number(smoothing) || 0.08, 0.02, 0.35);
  const safePerspective = clamp(Number(perspective) || 1000, 300, 2000);
