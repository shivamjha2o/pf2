      ...
      {isOpen && (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
          ...
          <motion.div
            ...
            className="relative w-full max-w-2xl bg-white border-3 sm:border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] sm:shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] rounded-2xl sm:rounded-3xl p-6 sm:p-8 z-10 my-8 max-h-[90vh] overflow-y-auto text-black"
          >
            ...
            <div className="space-y-6">
              {/* Executive Overview */}
              <div>
                <h4 className="text-xs font-bold uppercase text-gray-500 tracking-wider mb-1.5">
                  📌 Project Overview
                </h4>
                ...
                <div className="space-y-2">
                  {project.workflow.map((step, idx) => (
                    <div key={idx} className="flex items-start gap-2.5 text-xs sm:text-sm font-medium text-gray-800">
                      <span className="w-5 h-5 rounded-full bg-black text-white font-mono text-[10px] font-bold flex items-center justify-center shrink-0 mt-0.5">
                        {idx + 1}
                      </span>
                      <span>{step}</span>
                    </div>
              ...
              <div>
                <h4 className="text-xs font-bold uppercase text-gray-500 tracking-wider mb-2">
                  🛠️ Exact Tools & Stack Used
                </h4>
                <div className="flex flex-wrap gap-2">
                  {project.tools.map((tool) => (
                    <span
                      key={tool}
                      className="bg-white hover:bg-[#B8FF65] text-black font-extrabold text-xs px-3 py-1.5 border-2 border-black rounded-lg shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-colors"
                    >
                      {tool}
                    </span>
                  ))}
              ...
              <div>
                ...
                <ul className="space-y-1.5 list-disc list-inside text-xs sm:text-sm font-medium text-gray-800">
              ...
              {/* Action Buttons */}
              <div className="pt-4 border-t-2 border-black flex flex-wrap gap-3">
                <a
